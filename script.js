...
// Ajoutez ce code au fichier JS principal de votre frontend
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (refreshButton) {
      refreshButton.addEventListener('click', async function() {
        try {
          ...
          // Appeler l'API
          const response = await fetch('https://votre-backend-url.vercel.app/api/tokens/refresh', {
            method: 'POST',
            headers: {
              'Content-Type': 'application/json'
            },
            credentials: 'include' // Pour envoyer les cookies d'authentification si nécessaire
          });
          
          const data = await response.json();
          
          if (data.success) {
            // Rafraîchir les données affichées (recharger la page ou mettre à jour le DOM)
            alert('Data refreshed successfully!');
            window.location.reload(); // Option simple pour recharger la page
            
            // Alternative: appeler une fonction qui recharge juste les données sans recharger la page
      ...
      // Afficher le bouton pour les admins (vous avez probablement déjà cette logique)
      if (isUserAdmin()) {
  ...
  // Fonction d'exemple pour vérifier si l'utilisateur est admin
  function isUserAdmin() {
    // Implémentez votre logique pour vérifier si l'utilisateur est admin
    // Par exemple, vérifier un token JWT ou une variable globale
